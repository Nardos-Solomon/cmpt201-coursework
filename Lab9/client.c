...
port number). address is 127.0.0.1 and port 8000
2. Is it UDP or TCP? How do you know?
  i think it is TCP because it uses connect() but udp is a connectionless
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? the user enters data that is read and
then stored in the buffer. Can tell when it says  while ((num_read =
read(STDIN_FILENO, buf, BUF_SIZE)) > 1)
4. How does the client program end? How can you tell that in the code?
  when read(STDIN_FILENO< buf, BUF_SIZE) is less than 1.
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
